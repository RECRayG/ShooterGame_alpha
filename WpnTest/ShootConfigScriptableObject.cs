using UnityEngine;
using System.Linq;
using static Unity.Burst.Intrinsics.X86;
using static UnityEngine.Rendering.DebugUI.Table;
//using UnityEditor.PackageManager.UI;

namespace Guns
{
    // То, как стреляет оружие
    [CreateAssetMenu(fileName = "Shoot Config", menuName = "Guns/Shoot Config", order = 2)]
    public class ShootConfigScriptableObject : ScriptableObject
    {
        /*public bool IsHitscan = true;
        public Bullet BulletPrefab;
        public float BulletSpawnForce = 100;*/
        public LayerMask HitMask; // Маска попадания, на которую будет направлен луч
        public float FireRate = 0.25f; // Задержка между выстрелами
        public int CountOfBullet = 1; // Количество пуль от выстрела
        public BulletSpreadType SpreadType = BulletSpreadType.Simple; // Тип отдачи
        public float RecoilRecoverySpeed = 1f; // Скорость восстановления после отдачи
        public float MaxSpreadTime = 1f; // Максимальное время отдачи

        private ThirdPersonShootController thirdPersonShootController; // Ссылка на контроллер персонажа
        private Transform spawnBulletPosition; // Ссылка на объект, содержащий позицию начала выстрела

        /*public ShootType ShootType = ShootType.FromGun;*/

        /// <summary>
        /// When <see cref="SpreadType"/> = <see cref="BulletSpreadType.Simple"/>, this value is used to compute the bullet spread.
        /// This defines the <b>Maximum</b> offset in any direction that a single shot can offset the current forward of the gun. 
        /// The range is from -x -> x, -y -> y, and -z -> z. 
        /// </summary>
        [Header("Simple Spread")]
        public Vector3 Spread = new Vector3(0.1f, 0.1f, 0.1f); // Вектор разброса
        [Header("Texture-Based Spread")]
        /// <summary>
        /// Multiplier applied to the vector from the center of <see cref="SpreadTexture"/> and the chosen pixel. 
        /// Smaller values mean less spread is applied.
        /// </summary>
        [Range(0.001f, 5f)]
        public float SpreadMultiplier = 0.1f; // Множитель разброса (переменная, на которую увеличивается разброс при каждом выстреле)
        /// <summary>
        /// Weighted random values based on the Greyscale value of each pixel, originating from the center of the texture is used to calculate the spread offset.
        /// For more accurate guns, have strictly black pixels farther from the center of the image. 
        /// For very inaccurate weapons, you may choose to define grey/white values very far from the center 
        /// </summary>
        public Texture2D SpreadTexture; // Текстура разброса при отдаче

        /**
         * Calculates and returns the offset from "forward" that should be applied for the bullet
         * based on <param name="ShootTime"/>. The closer to <see cref="MaxSpreadTime"/> this is, the
         * larger area of <see cref="SpreadTexture"/> is read, or wider range of <see cref="Spread"/>
         * is used, depending on <see cref="SpreadType"/>
         */
        public Vector3 GetSpread(ShootConfigScriptableObject ShootConfig, float ShootTime = 0)
        {
            thirdPersonShootController = GameObject.FindGameObjectWithTag("Player").GetComponent<ThirdPersonShootController>();
            spawnBulletPosition = GameObject.FindGameObjectWithTag("SpawnBulletPistol").GetComponent<Transform>();
            Vector3 spread = Vector3.zero;

            // Если отдача простая, то выстрел без отдачи, иначе выстрел с исп. текстуры разброса
            if (SpreadType == BulletSpreadType.Simple)
            {
                /*spread = Vector3.Lerp(
                    Vector3.zero,
                    new Vector3(
                        Random.Range(-Spread.x, Spread.x),
                        Random.Range(-Spread.y, Spread.y),
                        Random.Range(-Spread.z, Spread.z)
                    ),
                    Mathf.Clamp01(ShootTime / MaxSpreadTime)
                );*/

                spread = Vector3.Lerp(Vector3.zero,
                            new Vector3(
                                Random.Range(-Spread.x,
                                            Spread.x
                                            ),
                                Random.Range(-Spread.y,
                                            Spread.y
                                            ),
                                Random.Range(-Spread.z,
                                            Spread.z
                                            )
                                        ), 
                            Mathf.Clamp01(ShootTime / MaxSpreadTime)
                            );
            }
            else if (SpreadType == BulletSpreadType.TextureBased)
            {
                spread = GetTextureDirection(ShootTime);
                spread *= SpreadMultiplier;
            }

            return spread;
        }

        /// <summary>
        /// Reads provided <see cref="SpreadTexture"/> and uses a weighted random algorithm
        /// to determine the spread. <param name= "ShootTime" /> indicates how long the player
        /// has been shooting, larger values, closer to <see cref = "MaxSpreadTime" /> will sample
        /// larger areas of the texture
        /// </summary>         
        // Выбор центральной точки внутри текстуры, которая со временем будет случайным образом перемещаться, т.е.
        // чем дольше длится выстрел, тем больше квадрат(круг) разброса: [], в котором выбирается случайная точка 
        private Vector2 GetTextureDirection(float ShootTime)
        {
            // 2D точка центра текущей текстуры
            Vector2 halfSize = new Vector2(SpreadTexture.width / 2f, SpreadTexture.height / 2f);

            // Значение квадрата отклонения в зависимотси от времени стрельбы (половина радиуса)
            int halfSquareExtents = Mathf.CeilToInt(Mathf.Lerp(0.01f, halfSize.x, Mathf.Clamp01(ShootTime / MaxSpreadTime)));

            // Координаты нижней левой точки
            int minX = Mathf.FloorToInt(halfSize.x) - halfSquareExtents;
            int minY = Mathf.FloorToInt(halfSize.y) - halfSquareExtents;

            // Выбор получившегося прямоугольника из текстуры с цветовым массивом (оттенки серого)
            Color[] sampleColors = SpreadTexture.GetPixels(
                minX,
                minY,
                halfSquareExtents * 2,
                halfSquareExtents * 2
            );

            // Преобразование всех полученных цветов в оттенки серого
            // (значение от 0 до 1, в зависимости от степени белезны пикселя)
            float[] colorsAsGrey = System.Array.ConvertAll(sampleColors, (color) => color.grayscale);
            // Вычислене общего значения серго
            float totalGreyValue = colorsAsGrey.Sum();

            // Выбор случайного значения от 0 до вычисленного значения серого
            float grey = Random.Range(0, totalGreyValue);

            // Проход по всем цветам как по серому
            int i = 0;
            for (; i < colorsAsGrey.Length; i++)
            {
                // Если случайное значение серого <= 0 после вычитания определённого цвета, значит, индекс нужной точки найдена
                grey -= colorsAsGrey[i];
                if (grey <= 0)
                {
                    break;
                }
            }

            // Вычисление позиции, основанной на выбранной точке разброса от нижней левой точки координат
            int x = minX + i % (halfSquareExtents * 2);
            int y = minY + i / (halfSquareExtents * 2);

            Vector2 targetPosition = new Vector2(x, y);

            // Вычисление направления пули
            Vector2 direction = (targetPosition - new Vector2(halfSize.x, halfSize.y)) / halfSize.x;

            return direction;
        }
    }
}